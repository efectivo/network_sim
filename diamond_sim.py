...
class DiamondRandomSrc(patterns.PatternIfc):
    ...
    def invoke(self, curr_cycle):
        ...
        num_packets = np.random.poisson(self.packet_num_in_cycle)
        return [invoke_one() for _ in range(num_packets)]

for L in xrange(1, 40):
#for L in [20,]:
    print L
    ...
    for packet_num_in_cycle in [1.1, 1.5, 2]:
        ...
        for _ in range(test_num):
            ...
            test2 = tested_unit.Test('2s_odd_even', next_step_down_hill.TwoStepsDownHill(divide_by_fanout=True))
            tests = [test1, test2]
            #tests = [test2]
